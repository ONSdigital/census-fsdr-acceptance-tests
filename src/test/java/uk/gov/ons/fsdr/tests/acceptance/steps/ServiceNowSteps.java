...
public class ServiceNowSteps {
  ...
  @Then("the employee {string} with closing report id {string} is correctly created in ServiceNow with {string} and status {string}")
  public void the_employee_is_correctly_create_in_ServiceNow_with(String employeeId, String crId, String roleId, String status) {
    ...
    String expectedMessageRootNode =
        "\"location\":\"London\",\"first_name\":\"Fransico\",\"last_name\":\"Buyo\",\"u_preferred_name\":null,\"u_badge_number\":null,\"u_line_manager\":\"Bob Jones\",\"employee_number\":\""
            + roleId
            + "\",\"u_job_role_2\":null,\"u_contract_start_date\":\"[0-9-]{10}\",\"u_contract_end_date\":\"[0-9-]{10}\",\"u_employment_status\":\"" + status + "\",\"zip\":\"FA43 1AB\",\"u_ons_id\":\"fransico.buyo[0-9]{2}@domain\",\"u_ons_device_number\":null,\"home_phone\":null,\"mobile_phone\":\"0987654321\",\"active\":true,\"user_name\":\"fransico.buyo[0-9]{2}@domain\"";

    assertThat(create).containsPattern(expectedMessageRootNode);
  }

  @Then("the employee {string} with closing report id {string} is correctly updated in ServiceNow with {string} and name {string} and number {string} and status {string}")
  public void the_employee_is_correctly_updated_in_ServiceNow_with(String id, String crId, String roleId, String name, String phoneNumber, String status) {
    checkSentEmployee(id+crId, roleId, name, phoneNumber, null, status);
  }

  @Then("the employee {string} with closing report id {string} is correctly updated in ServiceNow with {string} and name {string} and number {string} and contract start date {string} and status {string}")
  public void the_employee_is_correctly_updated_in_ServiceNow_with(String id, String crId, String roleId, String name, String phoneNumber, String contractStartDate, String status) {
    checkSentEmployee(id+crId, roleId, name, phoneNumber, contractStartDate, status);
  }

  public void checkSentEmployee(String id, String roleId, String name, String phoneNumber, String contractStartDate, String status) {
    ...
    expectedMessageRootNode = "\"location\":\"London\",\"first_name\":\"" + name
        ...
        + roleId
        + "\",\"u_job_role_2\":null,\"u_contract_start_date\":\"" + contractStartDate + "\",\"u_contract_end_date\":\"" + LocalDate.now().plusDays(5)+"\",\"u_employment_status\":\"" + status + "\",\"zip\":\"FA43 1AB\",\"u_ons_id\":\"fransico.buyo[0-9]{2}@domain\",\"u_ons_device_number\":"
